...
TEST_CASE("Polyfills/type_traits") {
  ...
  SUBCASE("is_array") {
    CHECK((is_array<const char*>::value == false));
    CHECK((is_array<const char[]>::value == true));
    CHECK((is_array<const char[10]>::value == true));
  }
  ...
  SUBCASE("remove_cv") {
    CHECK((is_same<remove_cv_t<const int>, int>::value == true));
    CHECK((is_same<remove_cv_t<volatile int>, int>::value == true));
    CHECK((is_same<remove_cv_t<const volatile int>, int>::value == true));
    CHECK((is_same<remove_cv_t<int>, int>::value == true));
    CHECK((is_same<remove_cv_t<decltype("toto")>, decltype("toto")>::value ==
           true));
  }

  SUBCASE("decay") {
    CHECK((is_same<decay_t<int>, int>::value == true));
    CHECK((is_same<decay_t<int&>, int>::value == true));
    CHECK((is_same<decay_t<int&&>, int>::value == true));
    CHECK((is_same<decay_t<int[]>, int*>::value == true));
    CHECK((is_same<decay_t<int[10]>, int*>::value == true));
    CHECK((is_same<decay_t<decltype("toto")>, const char*>::value == true));
  }
}
