...
TEST_CASE("ARDUINOJSON_STRING_LENGTH_SIZE == 1") {
  ...
  SUBCASE("set(std::string)") {
    SUBCASE("returns true if len <= 255") {
    ...
    SUBCASE("returns false if len >= 256") {
  ...
  SUBCASE("set(MsgPackBinary)") {
    SUBCASE("returns true if size <= 253") {
    ...
    SUBCASE("returns false if size >= 254") {
  ...
  SUBCASE("set(MsgPackExtension)") {
    SUBCASE("returns true if size <= 252") {
    ...
    SUBCASE("returns false if size >= 253") {
      auto str = std::string(253, '?');
      auto result = doc.set(MsgPackExtension(1, str.data(), str.size()));
  ...
  SUBCASE("deserializeJson()") {
    SUBCASE("returns Ok if string length <= 255") {
    ...
    SUBCASE("returns NoMemory if string length >= 256") {
  ...
  SUBCASE("deserializeMsgPack()") {
    ...
    SUBCASE("returns Ok if binary size <= 253") {
    ...
    SUBCASE("returns NoMemory if binary size >= 254") {
    ...
    SUBCASE("returns Ok if extension size <= 252") {
    ...
    SUBCASE("returns NoMemory if binary size >= 253") {
