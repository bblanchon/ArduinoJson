...
TEST_CASE("ARDUINOJSON_STRING_LENGTH_SIZE == 2") {
  ...
  SUBCASE("set(std::string)") {
    SUBCASE("returns true if len <= 65535") {
    ...
    SUBCASE("returns false if len >= 65536") {
  ...
  SUBCASE("set(MsgPackBinary)") {
    ...
    SUBCASE("returns false if size >= 65533") {
  ...
  SUBCASE("set(MsgPackExtension)") {
    SUBCASE("returns true if size <= 65531") {
    ...
    SUBCASE("returns false if size >= 65532") {
      auto str = std::string(65532, '?');
      auto result = doc.set(MsgPackExtension(1, str.data(), str.size()));
  ...
  SUBCASE("deserializeJson()") {
    SUBCASE("returns Ok if string length <= 65535") {
    ...
    SUBCASE("returns NoMemory if string length >= 65536") {
  ...
  SUBCASE("deserializeMsgPack()") {
    ...
    SUBCASE("returns Ok if binary size <= 65532") {
    ...
    SUBCASE("returns NoMemory if binary size >= 65534") {
    ...
    SUBCASE("doesn't overflow if binary size == 0xFFFF") {
    ...
    SUBCASE("returns Ok if extension size <= 65531") {
    ...
    SUBCASE("returns NoMemory if extension size >= 65532") {
